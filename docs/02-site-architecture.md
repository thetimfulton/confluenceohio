...
| Utqiaġvik (Barrow) | Full case study — vote, process, outcome, lessons | 300 |
...
### 4.1 The Petition Path — Every Page Leads to `/sign`

Every page on the site must have at least one clear path to the petition. Here is how petition CTAs appear across the site:

| Page | CTA Placement | CTA Type |
|------|--------------|----------|
| Homepage | Hero button + signature counter section + mobile sticky bar | Primary button, counter widget, sticky bar |
| /the-case (all sub-pages) | End-of-content inline banner + mobile sticky bar | Inline banner with counter |
| /voices | Inline banner between voice grid rows + mobile sticky bar | Inline banner |
| /voices/[slug] | End-of-article banner + mobile sticky bar | Inline banner |
| /volunteer | Top section (for non-signers) + mobile sticky bar | Conditional banner |
| /donate | Sidebar or top mention (for non-signers) | Subtle text link |
| /about | End-of-page banner + mobile sticky bar | Inline banner |
| /press | Not prominent (journalists are not the signing audience) | Footer only |
| /blog/[slug] | End-of-article banner + mobile sticky bar | Inline banner with counter |
| /faq | End-of-page banner + inline after key Qs + mobile sticky bar | Inline banner |

**CTA copy variations** (rotate to reduce banner blindness):
- "Add Your Name" (default)
- "Join [X] Ohioans"
- "Be signer #[next number]"
- "Help us reach [next milestone]"
- "22,000 signatures. We need yours."

### 4.2 Post-Signature Conversion Funnel

This is the sequence a signer encounters after submitting the petition form. Each step is designed to capitalize on peak engagement while respecting attention decay.

```
┌─────────────────────┐
│   SIGN PETITION      │  ← Peak commitment moment
└──────────┬──────────┘
           ▼
┌─────────────────────┐
│  THANK YOU + NUMBER  │  "You're signer #4,217!"
│  (Emotional peak)    │  Confetti animation
└──────────┬──────────┘
           ▼
┌─────────────────────┐
│  SHARE              │  "Help us reach 5,000 — share with 3 friends"
│  (Highest leverage)  │  Pre-populated: Facebook, X, WhatsApp, Email, Copy
│                      │  Each share includes unique referral code /r/[code]
└──────────┬──────────┘
           ▼
┌─────────────────────┐
│  DONATE              │  "Fund the path to 22,000 signatures"
│  (Financial ask)     │  Suggested: $5, $10, $25, $50
│                      │  ActBlue link with signature refcode
└──────────┬──────────┘
           ▼
┌─────────────────────┐
│  VOLUNTEER           │  "Want to do more?"
│  (Deepest commitment)│  Quick role interest selector (checkboxes)
│                      │  "We'll be in touch"
└──────────┬──────────┘
           ▼
┌─────────────────────┐
│  EMAIL VERIFY        │  "Check your inbox to verify your signature"
│  (Housekeeping)      │  Resend link available
└─────────────────────┘
```

**Implementation notes:**
- Steps 2–5 appear as scrollable sections on a single `/sign/thank-you` page (not separate pages — reduce friction)
- Share is placed BEFORE donate because sharing has zero cost and highest viral leverage
- Donate amounts link directly to ActBlue (external) with refcode tracking
- Volunteer "interest" is a lightweight checkbox capture, not a full form — the full form is on `/volunteer`
- The entire post-signature flow works without JavaScript (server-rendered sections) but is enhanced with scroll animations and confetti via JS

### 4.3 Ladder of Engagement

The campaign defines seven tiers of engagement, each with a clear path to the next level:

```
TIER 7:  LEAD
         Campaign leadership, petition committee member
         ↑ Recruited from active volunteers
         
TIER 6:  DONOR (Recurring)
         Monthly sustainer via ActBlue
         ↑ Prompted after one-time donation or deep volunteer engagement

TIER 5:  DONOR (One-time)
         One-time ActBlue contribution
         ↑ Post-signature flow, email nurture, event asks

TIER 4:  VOLUNTEER
         Active campaign participant (any role)
         ↑ Post-signature flow, email nurture, events

TIER 3:  SUBSCRIBER
         Email list member (may or may not have signed)
         ↑ Petition auto-subscribe, footer signup, blog signup, event signup

TIER 2:  SIGNER
         Petition signatory (verified Ohio resident)
         ↑ Primary conversion from any page

TIER 1:  VISITOR
         Anyone who hits the site
         ↑ Organic search, social media, press, referral, ads
```

**Tier transitions and triggers:**

| From | To | Trigger |
|------|----|---------|
| Visitor → Signer | Petition form submission on `/sign` | Organic conviction, CTA clicks, social proof |
| Signer → Subscriber | Auto-opt-in during signature (checkbox, pre-checked per CAN-SPAM) | Signature form includes email opt-in |
| Signer → Sharer | Post-signature share panel on `/sign/thank-you` | Emotional peak + pre-populated share links |
| Signer → Donor | Post-signature donate prompt, email nurture sequence (Day 3, Day 7) | Gratitude + urgency framing |
| Signer → Volunteer | Post-signature volunteer teaser, email nurture (Day 5), events | "Do more" framing |
| Subscriber → Signer | Email nurture sequence (non-signer subscribers) | 3-email drip: story → urgency → ask |
| Volunteer → Donor | In-person ask at events, email after first volunteer activity | Reciprocity + deeper investment |
| Donor → Recurring | Email follow-up after one-time donation (Day 14) | "Make it monthly" pitch |
| Volunteer → Lead | Direct outreach from campaign staff | Demonstrated commitment + skills match |

### 4.4 Email Nurture Sequences (Brevo Automation)

**Sequence 1: Post-Signature Welcome (all signers)**
| Day | Email | Goal |
|-----|-------|------|
| 0 | "Welcome, signer #[X]!" — confirmation, share links, campaign overview | Verify email, drive shares |
| 3 | "Here's why [X] people signed this week" — social proof + progress update | Reinforce decision, prompt sharing |
| 5 | "Want to do more?" — volunteer spotlight, role descriptions | Volunteer conversion |
| 7 | "Your signature matters — here's what happens next" — process explanation + donate ask | Donation conversion |
| 14 | "Milestone update" — progress toward 22,000 + share ask | Re-engagement, sharing |

**Sequence 2: Non-Signer Subscriber**
| Day | Email | Goal |
|-----|-------|------|
| 0 | "Thanks for joining" — welcome, manifesto excerpt, link to /the-case | Warm introduction |
| 3 | "The naming story" — link to /the-case/history | Education |
| 7 | "Join [X] Ohioans who've signed" — social proof, direct petition link | Petition conversion |

**Sequence 3: Post-Volunteer Signup**
| Day | Email | Goal |
|-----|-------|------|
| 0 | "Welcome to the team!" — role confirmation, what to expect | Set expectations |
| 3 | "Your first action" — specific task based on selected role | Activation |
| 14 | "You've made a difference" — impact report + donate ask | Donation conversion |

### 4.5 Referral System Design

Every signer receives a unique referral code at the thank-you step.

**URL structure:** `confluenceohio.org/r/[code]`

**Referral flow:**
1. Signer shares link (e.g., `confluenceohio.org/r/TF4K2`) via any channel
2. Friend clicks → server records click + platform attribution → 301 redirect to `/sign?ref=TF4K2`
3. Friend signs → `referral_code` field on signature record → `conversions` counter incremented on referral record
4. Original signer notified via Brevo transactional email: "Your friend [first name] just signed! You've referred [X] signers." (Inngest event triggers email send; batched if multiple referrals arrive within a short window to avoid spam)

**Referral leaderboard (future feature — Phase 2):**
- Top referrers displayed on a public leaderboard
- Gamification: milestones at 3, 10, 25, 50 referrals
- Social proof: "Top referrer this week: [First Name] from [City] — 14 referrals"

### 4.6 Signature Counter Milestones

The live counter uses milestone framing to create urgency:

| Milestone | Label | Trigger |
|-----------|-------|---------|
| 100 | "First 100!" | Confetti on signer #100 |
| 500 | "500 strong" | |
| 1,000 | "1,000 Ohioans and counting" | Blog post, social blast |
| 2,500 | "2,500 — over 10% of the way!" | |
| 5,000 | "5,000 signatures" | Press release |
| 10,000 | "Halfway there" | Major campaign moment |
| 15,000 | "15,000 — the finish line is in sight" | |
| 20,000 | "20,000 — almost there!" | Urgency push |
| 22,000 | "GOAL REACHED — 22,000 signatures!" | Major press release, celebration |
...
| 20 | **Precedent city imagery** | Licensed photos of Utqiaġvik, Cincinnati riverfront, Atlanta skyline, Mumbai — illustrating the "cities that changed" narrative | `/the-case/precedents` | Standard editorial license |
...
### Prompt for `docs/site-architecture.md`

```
Create the file `docs/site-architecture.md` in the repository root.

This document defines the complete site architecture for confluenceohio.org. Copy the full contents of artifact 02-site-architecture.md — specifically:

1. Section 1: Site Map — the full URL hierarchy with 14 static routes, 3 dynamic route patterns, and 2 post-action routes. Include the URL conventions (lowercase, hyphenated, canonical URLs, HTTPS, referral redirect behavior).

2. Section 2: Page-by-Page Specifications — all 23 page specs, each containing: purpose, user intent, primary CTA, conversion goal, secondary CTAs, content outline table, key UI components list, SEO target keywords, OG image concept, and JSON-LD requirements where applicable. This section is the source of truth for what every page does and contains.

3. Section 3: Navigation Design — desktop primary nav (with "The Case" dropdown and accent "Sign the Petition" button), mobile nav (hamburger + persistent sticky bottom CTA bar), footer nav (3-column with email signup and social links for Facebook, X/Twitter, and Instagram), and sub-navigation for /the-case/* pages.

4. Section 5: Photography and Visual Asset Shot List — 20 assets across 3 priority tiers (P1 launch-critical, P2 within 30 days, P3 ongoing), with specs and sourcing notes.

Format as clean Markdown with H1 for the document title, H2 for each major section, and H3 for each page specification. Use tables for content outlines and component lists where the source document uses them.

Do not modify any content — this is the approved site architecture. Cross-reference artifact 01-messaging-framework.md for messaging alignment.
```

### Prompt for `docs/conversion-architecture.md`

```
Create the file `docs/conversion-architecture.md` in the repository root.

This document defines the conversion strategy for confluenceohio.org. Copy the full contents of Section 4 from artifact 02-site-architecture.md, specifically:

1. Section 4.1: The Petition Path — table showing CTA placement on every page, CTA copy variations for rotation.

2. Section 4.2: Post-Signature Conversion Funnel — the 5-step flow from signature to email verification, with implementation notes (single scrollable page, JS progressive enhancement, share before donate rationale).

3. Section 4.3: Ladder of Engagement — 7 tiers (Visitor through Lead) with tier transition triggers table.

4. Section 4.4: Email Nurture Sequences — 3 Brevo automation sequences (post-signature, non-signer subscriber, post-volunteer) with day-by-day tables.

5. Section 4.5: Referral System Design — URL structure, referral flow (4 steps), referral leaderboard concept for Phase 2.

6. Section 4.6: Signature Counter Milestones — milestone table with labels and triggers from 100 to 22,000.

Format as clean Markdown. Use ASCII flow diagrams for the post-signature funnel and the ladder of engagement (as they appear in the source). Use tables for the CTA placement map, tier transitions, email sequences, and milestones.

This document will be the primary reference for implementing the conversion and engagement systems in the Next.js app and Brevo automation.
```

### Prompt for downstream artifacts

```
Read `docs/site-architecture.md` and `docs/conversion-architecture.md` from the repository. These are the approved Prompt 2 outputs. Also confirm you have `docs/messaging-framework.md` (Prompt 1) loaded. All three are required before proceeding to Prompt 3.
```
